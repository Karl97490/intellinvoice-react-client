...
import { Datepicker, DropdownItem, TextInput, Button } from "flowbite-react";
...
const Invoices = () => {
  ...
  // Recalculer la pagination quand invoices change
  useEffect(() => {
    if (invoices && invoices.length > 0) {
      const totalPages = Math.ceil(invoices.length / pagination.limit);
      setPagination((prev) => ({
        ...prev,
        totalPages,
                  ...
                  <tbody>
                    {!invoices.length ? (
                      <tr>
                        <th
                          colSpan={6}
                          className="text-center py-5 text-gray-400 text-lg font-medium"
                        >
                          Not invoices yet.
                        </th>
                      </tr>
